...
class TestSendRpc:
    ...
    @pytest.mark.parametrize('correlation_id, expected', [
        ('my-correlation-id', 'my-correlation-id'),
        (None, RESPONSE_CORR_ID),  # generated by the library
    ])
    async def test_correlation_id(self, connection, correlation_id, expected):
        ...
        rpc.channel.basic_publish.assert_called_once_with(
            exchange_name='', routing_key=TEST_DESTINATION, payload=b'payload',
            properties={'reply_to': RANDOM_QUEUE, 'correlation_id': expected})
